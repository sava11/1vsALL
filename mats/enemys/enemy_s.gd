...
@export var prg_bar:bool=false
@export_range(0,100) var dif:float=1
@export var run_speed:float=25.0
@export_range(-1,1) var mobility:float=0
...
@export var doing_chance_list={"attack":1.0}
...
@export_range(-999999999,999999999) var mny_from=0
...
func set_param(f,t,d:float=0):
	return fnc._with_dific(randf_range(f,t),d)
func pre_ready():pass
...
func _ready():
	...
	var def=0
	if !elite:
		life_points=set_param(life_points_from,life_points_to,dif)
		damage=set_param(damage_from,damage_to,dif)
		def=set_param(defence_from,defence_to,dif)
	else:
		life_points=set_param(life_points_from,life_points_to,dif)*1.5
		damage=set_param(damage_from,damage_to,dif)*1.5
		def=set_param(defence_from,defence_to,dif)*1.25
	...
	past_ready()
	if prg_bar:
		pb=preload("res://mats/enemys/hpBar/pb.tscn").instantiate()
...
func thinker():
	if think_timer>=think_time and current_action==0:
		think_timer=0
		current_action=fnc._with_chance_ulti(doings_times)+1
func _draw():
...
func _integrate_forces(st):
	...
	if prg_bar==true and is_instance_valid(pb):
		pb_upd()
	pre_status()
...
func get_visual_contact_with(obj):
	return $get_hero_body/r.get_collider()==obj and bs.has(hero)
func pre_status():
...
func find_status(_delta:float):
	...
	can_attack=get_visual_contact_with(hero) and get_sqrt(hero)<attack_range
	if state!="wait_anim" and state!="d":
		if get_visual_contact_with(hero) and get_sqrt(hero)<attack_range:
			cur_target_pos=hero.global_position
			if to_idle() and state!="wait_anim":
				state="i"
		if !can_attack and get_sqrt(hero)>=save_sqrt and get_sqrt(hero)>attack_range and to_idle():
			cur_target_pos=hero.global_position
			state="r"
		in_status_think()
...
func delete():
	...
	if mny_from!=mny_to and exp_from!=exp_to:
		...
		if v.type==0:
			v.value=fnc._with_dific(clamp(randi_range(mny_from,mny_to),0,999999999),dif)
		else:
			v.value=fnc._with_dific(clamp(randi_range(exp_from,exp_to),0,999999999),dif)
		get_parent().add_child.call_deferred(v)
		v.set_deferred("global_position",global_position)
	state="d"
