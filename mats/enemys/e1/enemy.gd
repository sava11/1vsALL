...
func _ready():
	...
	for e in attacks_chance_list.keys():
		doings_times.append(attacks_chance_list[e])
...
func to_idle():
	return na.is_navigation_finished() and (get_sqrt(hero)>save_sqrt and get_sqrt(hero)<=save_sqrt*1.5)
func find_status(_delta:float):
	...
	if state!="wait_anim" and state!="d":
		...
		if get_visual_contact_with(hero) and get_sqrt(hero)<save_sqrt:
			cur_target_pos=hero.global_position
			if state!="a" and state!="wait_anim":
				state="i"
		in_status_think()
